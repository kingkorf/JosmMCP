...
* `find_duplicate_nodes` – groups of nodes at the same position (or within `tolerance_m`) in a bbox or the whole layer, with tags, parent ways **and those ways' tags**, and whether merging would conflict. `mergeable` means no conflict, not that merging is right: a river touching an administrative boundary is mergeable and should be left alone
